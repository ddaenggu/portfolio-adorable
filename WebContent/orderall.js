...
$(document).ready(function() {
	...
	$(".pointbox").focusout(function() {
		if (od.point.value < 5000 || od.usepoint.value < 5000 || parseInt(od.usepoint.value) > parseInt(od.point.value)) {
			// od.point.value = 내가 가지고 있는 포인트, od.usepoint.value = 내가 사용할 포인트
			alert("포인트는 5000원 이상부터 사용 가능합니다.");
			od.usepoint.value = "";
			document.getElementById("usepointzone").innerText = 0;
			document.getElementById("sumprice").innerText =
			 parseInt(document.getElementById("allprice").innerText) 
		+ parseInt(document.getElementById("deliveryprice").innerText) 
		- parseInt(document.getElementById("usepointzone").innerText);
			// 총 상품금액 + 배송비 - 포인트사용 = 결제금액
		}
		else {
			document.getElementById("usepointzone").innerText = od.usepoint.value;
			document.getElementById("sumprice").innerText =
			 parseInt(document.getElementById("allprice").innerText) + parseInt(document.getElementById("deliveryprice").innerText) - parseInt(document.getElementById("usepointzone").innerText);
			// 총 상품금액 + 배송비 - 포인트사용 = 결제금액
		}
	});
