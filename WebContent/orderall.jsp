...
<title>adorable</title>
	...
	<script type="text/javascript" src="orderall.js"></script>
...
%>
	...
	<%
	List<ModelProduct> list = (List<ModelProduct>)request.getAttribute("arr");
	List<Integer> quan = (List<Integer>)request.getAttribute("quantity");
	ModelProduct cust = (ModelProduct)request.getAttribute("m");
		
		%>
	
	<section>
	<div class="bigshape">
	<form name="od" action="insertallproduct.prod" method="post">
	...
	<input type="hidden" name="pro_code" value="<%= m.getPro_code() %>">
	<input type="hidden" name="pro_price" value="<%= m.getPro_price() %>" class="pro_price">
	<input type="hidden" name="quantity" value="<%= mq %>" class="quantity">
	<tr>
	<td class="imagezone">
	<img src="imgfile/<%= m.getPb_img1() %>" width="100px" height="100px">
	</td>
	<td class="iteminfozone">
	<div class="itemname"><%= m.getPro_name() %></div>
	옵션 : <%= m.getPro_color() %> / <%= m.getPro_size() %>
	</td>
	<td class="pricezone"><%= m.getPro_price() %> 원</td>
	<td class="quantityzone"><%= mq %> 개</td>
	...
	<%= cust.getName() %><br>
	<%= cust.getNum_front() %>-<%= cust.getNum_middle() %>-<%= cust.getNum_back() %><br>
	포인트 : <%= cust.getPoint() %> 원<br>
	<input type="hidden" name="point" value="<%= cust.getPoint() %>">
	...
	<%= cust.getZipcode() %> <br>
	<%= cust.getAddress() %> <br>
	<%= cust.getDetailaddress() %> <br>
	<%= cust.getExtraaddress() %>
	<input type="hidden" name="zipcode" value="<%= cust.getZipcode() %>">
	<input type="hidden" name="address" value="<%= cust.getAddress()%>">
	<input type="hidden" name="detailaddress" value="<%= cust.getDetailaddress() %>">
	<input type="hidden" name="extraaddress" value="<%= cust.getExtraaddress() %>">
	</div>
	</li>
	<li>
