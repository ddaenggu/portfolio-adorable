...
ModelLookbook m = lb.selectDetail(lb_num);
String [] proname = m.getProname().split(",");
	...
	<a href="lookbook.jsp"><img src="image/lookbooklogo.jpg" width="100%"></a>
		...
		<form name="lb" action="processlookbookupdate.jsp" method="post">
		<input type="hidden" name="lb_num" value="<%= m.getLb_num() %>">
		<div class="writetitle2">
		<input type="text" class="writetitle" placeholder="제목을 입력해주세요" name="lb_title" value="<%= m.getLb_title() %>">
		</div>
		<div class="writecontent2">
		<textarea name="lb_content" class="writecontent" placeholder="내용을 입력해주세요"><%= m.getLb_content() %></textarea>
		</div>
		...
		for (int i = 0; i < arr.size(); i++) {
			...
			%>
			<span class="ck">
		<input type="checkbox" name="proname" value="<%= ml.getPro_name() %>"> <%= ml.getPro_name() %>
			</span>
			<%
		}
		%>
		...
		<div class="writeok">
		<input type="submit" value="수정하기" class="writebtn" onclick="return writeok()">
		</div>
		</form>
		</div>
	</section>
