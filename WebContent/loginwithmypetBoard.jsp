...
<img src="image/withmypetlogo.png" width="100%">
	...
	<div class="boardshape">
				...
				<table class="boardtable">
					...
					%>
						<tr>
							...
							if (id == null) {
								%>
							<td class="th_title"><a href="login.jsp"><%= articleList.get(i).getBoard_title() %></a></td>
								<%
							}
							else {
								%>
		...
		[이전]&nbsp;
		<%}else{ %>
		<a href="boardListSelect.board?page=<%=nowPage-1 %>">[이전]</a>&nbsp;
		<%} %>
		...
		[<%=a %>]
		<%}else{ %>
		<a href="boardListSelect.board?page=<%=a %>">[<%=a %>]
		</a>&nbsp;
		<%} %>
		<%} %>
		...
		[다음]
		<%}else{ %>
		<a href="boardListSelect.board?page=<%=nowPage+1 %>">[다음]</a>
		<%} %>
					</div>
					<%
					if (id != null) {
					...
					<div class="boardwrite">
						<button class="write" onclick="location.href='boardWriteForm.board'">글쓰기</button>
					</div>
					<div class="boardmywriting">
						<button class="lookmywriting" onclick="location.href='boardSelectMine.board'">내글보기</button>
					</div>
						<%
					}
					%>
